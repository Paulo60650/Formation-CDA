...
class Orders
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=customers::class, mappedBy="CustomersList")
     */
    private $CustomersID;

    public function __construct()
    {
        $this->CustomersID = new ArrayCollection();
    }
     ...
     */
    public function getCustomersID(): Collection
    {
        return $this->CustomersID;
    }

    public function addCustomersID(customers $customersID): self
    {
        if (!$this->CustomersID->contains($customersID)) {
            $this->CustomersID[] = $customersID;
            $customersID->setCustomersList($this);
        }

        return $this;
    }

    public function removeCustomersID(customers $customersID): self
    {
        if ($this->CustomersID->contains($customersID)) {
            $this->CustomersID->removeElement($customersID);
            // set the owning side to null (unless already changed)
            if ($customersID->getCustomersList() === $this) {
                $customersID->setCustomersList(null);
            }
        }
